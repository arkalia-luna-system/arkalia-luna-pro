...
class StateManager:
        ...
        try:
            ...
            # Décisions opposées
                ...
                contradictions.append(
                    f"Décisions divergentes: {reflexia_decision} vs {zeroia_decision}"
                )
            ...
            if contradictions:
                ...
                try:
                    ...
                    ark_logger.warning(
                        f"⚠️ Conflit IA détecté: {contradictions}",
                        extra={"arkalia_module": "zeroia"},
                    )
                    return True
            ...
            ark_logger.error(
                f"❌ Erreur vérification conflit: {e}", extra={"arkalia_module": "zeroia"}
            )
            return False
    ...
    def load_current_state(self) -> dict:
        ...
        try:
            if self.state_path.exists():
                ...
                ark_logger.warning(
                    f"État ZeroIA non trouvé: {self.state_path}", extra={"arkalia_module": "zeroia"}
                )
                return {
    ...
    def cleanup_old_logs(self, max_age_days: int = 30) -> None:
        ...
        try:
            ...
            with open(self.contradiction_log_path, encoding="utf-8") as f:
                lines = f.readlines()
