...
async def demo_complete_workflow():
    ...
    ark_logger.info(
        "🎯 Implémentation complète de tes recommandations :", extra={"arkalia_module": "scripts"}
    )
    ark_logger.info(
        "   ✅ 1. Réactions automatiques (7+ répétitions → pause)",
    ...
    ark_logger.info(
        "   ✅ 5. Mode Berserk/Recovery pour panics", extra={"arkalia_module": "scripts"}
    )
    ark_logger.info("")
    ...
    ark_logger.info(
        "🧪 Test de stress (patterns répétitifs + dégradation)...",
        extra={"arkalia_module": "scripts"},
    )
    ...
    ark_logger.info(
        f"   - Cycles totaux: {summary['global_stats']['total_cycles']}",
        extra={"arkalia_module": "scripts"},
    )
    ark_logger.info(
    ...
    ark_logger.info(
        f"   - Activations berserk: {summary['global_stats']['berserk_activations']}",
        extra={"arkalia_module": "scripts"},
    )
    ark_logger.info(
        f"   - Modules en quarantine: {len(summary['quarantined_modules'])}",
        extra={"arkalia_module": "scripts"},
    )
    ark_logger.info(
        f"   - Patterns détectés: {summary['global_stats']['patterns_detected']}",
        extra={"arkalia_module": "scripts"},
    )
    ...
    ark_logger.info(
        f"   - Timeline: {exports['timeline_file']}", extra={"arkalia_module": "scripts"}
    )
    ...
    ark_logger.info(
        f"📋 Timeline disponible: {summary['timeline_file']}", extra={"arkalia_module": "scripts"}
    )
...
def generate_heatmap_sample():
    ...
    ark_logger.info(
        "📊 Génération données heatmap pour Grafana...", extra={"arkalia_module": "scripts"}
    )
    ...
    ark_logger.info(
        f"📊 Données heatmap générées: {output_file}", extra={"arkalia_module": "scripts"}
    )
    ark_logger.info(f"   - {len(heatmap_data['heatmap_data'])} points de données")
    ark_logger.info(
        f"   - Résolution: {heatmap_data['summary']['resolution_minutes']} minutes",
        extra={"arkalia_module": "scripts"},
    )
